...
// Get the t-shirt element
const tshirt = document.getElementById('tshirt');

// Add a click event listener to the t-shirt element
tshirt.addEventListener('click', () => {
  // Add the t-shirt to the cart array
  cart.push('T-shirt');
  // Log a message to the console indicating the t-shirt was added to the cart
...
deleteButton.addEventListener('click', () => {
  // Remove the last item from the cart array
  const lastItem = cart.pop();
  // If the cart array is empty, log a message to the console indicating it's empty
  if (cart.length === 0) {
    console.log('The cart is now empty!');
  } else {
    // Otherwise, log a message to the console indicating the last item was removed from the cart
    console.log(`Item ${lastItem} removed from cart!`);
  }
});
